...
images_dir = ".\\randomtest"
templates_dir = ".\\templates"
result_filename = "scan_result.xlsx"
accepted_format = [".jpg", ".jpeg"]

# set to true to show result image and print text extracted from image
# makes execution slower, especially when working with larger images
verbose = True
# height and width of the result image
preferred_height = 600 # pixels, may vary depending on resolution
preferred_width = 900
...
with scandir(templates_dir) as it:
	for entry in it:
		...
		if splitext(template["path"])[1] not in accepted_format:
			continue

		# otherwise, load it into list of templates
